        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if any(self.model in name for name in model_names):
                    logger.info("Ollama model %s is available at %s", self.model, self.base_url)
                    return True
                else:
...
class OpenAIClient(BaseLLMClient):
    ...
    def _check_availability(self) -> bool:
        ...
        try:
            ...
            logger.error("OpenAI client initialization error: %s", e)
            return False
...
class LLMFactory:
    ...
    def get_best_available_client(
            self, preference_order: Optional[List[LLMProvider]] = None
...
def get_llm_client(provider: LLMProvider = LLMProvider.OLLAMA,
                   **config) -> Optional[BaseLLMClient]:
    """Convenience function to get an LLM client"""
    try:
        return llm_factory.get_client(provider, **config)
    except (TypeError, ValueError, ImportError) as e:
        logger.error("Failed to get LLM client for %s: %s", provider.value, e)
        return None
